...
#   IB_DOCKER_IMAGE   image name:tag        (default: micofe-build:1.0)
...
IB_DOCKER_IMAGE=${IB_DOCKER_IMAGE:-micofe-build:1.0}
...
if [ $# -eq 0 ]; then
	set -- "$IB_DOCKER_IMAGE" bash --rcfile /etc/micofe-build.bashrc -i
else
