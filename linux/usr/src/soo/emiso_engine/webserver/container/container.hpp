...
namespace container {
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_GET(const httpserver::http_request &req) {
            ...
            vector<string> namesFilter;

            if (!filtersArg.empty()) {
                Json::CharReaderBuilder readerBuilder;
                Json::Value filtersJsonValue;
                istringstream filtersJsonStream(filtersArg);
                ...
                if (!Json::parseFromStream(readerBuilder, filtersJsonStream, &filtersJsonValue, nullptr)) {
                    ...
                    cerr << LOG_PREFIX "Error parsing JSON!" << endl;
                }
...

            {
                ...
                for (auto it = info.begin(); it != info.end(); ++it) {
                    ...
                    if (!namesFilter.empty()) {
                        for (const auto &pattern : namesFilter) {
                            regex regexPattern(pattern);
                             if (regex_match("/" + it->second.name, regexPattern)) {
                                nameFound = true;
                                break;
                             }
                        }
                    ...
                    // Retrieve info about the image used to create the container
                    ImageInfo imageInfo;
                    _daemon->image.info(it->second.image, imageInfo);

                    // == Return info of the container ==

                    payload_json[idx]["Id"]       =  to_string(it->second.id);
                    ...
                    payload_json[idx]["NetworkSettings"]["Networks"]["bridge"]["NetworkID"]  = "7ea29fc1412292a2d7bba362f9253545fecdfa8ce9a6e37dd10ba8bee7129812",
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_POST(const httpserver::http_request &req) {
            string payload_str = "";
            Json::Value payload_json;
            int containerId;
            ...
            const string requestBody = string(req.get_content());

            // Retrieve the 'name' from request args
            for (const auto &arg : req.get_args()) {
                if (string(arg.first) == "name") {
                     containerName = string(arg.second);
                }
            }
            ...
            if (!jsonReader.parse(requestBody, jsonData)) {
                cout << "Failed to parse requestBody" << endl;
                // JSON parsing error
                // Return error message
            }

            string imageName = jsonData["Image"].asString();
            ...
            // build the response
            payload_json["Id"] = std::to_string(id);
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_POST(const httpserver::http_request &req) {
            ...
            string path = string(req.get_path()); 
            smatch match;
            regex rgx("(/v[0-9]+.[0-9]+)?/containers/([^/]+)/start");

            unsigned containerId;
            if (regex_match(path, match, rgx)) {
                // containerName = match[2];
                containerId = std::stoi(match[2].str());
            }

            cout << LOG_PREFIX "Start container based on '" << containerId << "' ID" << endl;
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_POST(const httpserver::http_request &req) {
            ...
            cout << "[WEBERVER] '" << req.get_path()  << "' (" << req.get_method() << ") called" << endl;
