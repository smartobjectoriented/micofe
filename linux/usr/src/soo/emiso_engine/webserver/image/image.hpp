...
namespace image {
    ...
    class HistoryHandler : public httpserver::http_resource {
    public:
        HistoryHandler(Daemon *daemon) : _daemon(daemon) {};
        ...
        std::shared_ptr<httpserver::http_response> render_GET(const httpserver::http_request &req) {
            ...
            for (auto it = info.begin(); it != info.end(); ++it) {
                ...
                payload_json[idx]["Id"]        = it->second.id;
                payload_json[idx]["Created"]   = it->second.date;
                payload_json[idx]["CreatedBy"] = "Micofe";
                payload_json[idx]["Tags"]      = Json::arrayValue;
                payload_json[idx]["Size"]      = it->second.size;
                payload_json[idx]["Comment"]   = "";
