...
int Image::createdTime(const std::string& filePath)
{
    try {
        ...
        if (fs::exists(filePath)) {
            ...
            // Get Unix timestamp
            std::time_t unix_ts = std::chrono::system_clock::to_time_t(sctp);
...
int Image::info(std::string name, ImageInfo &info)
{
    bool isID = false;
    std::string id_orig;

    // The 'name' parameter can be the image name of the image ID

    std::cout << "info a image '" << name << "' called" << std::endl;

    // 1. extracting the "code" from the 'name'
    auto pos = name.find(':');
    if (pos != std::string::npos) {
        id_orig = name.substr(pos + 1);

    } else {
        id_orig = this->calculateId(EMISO_IMAGE_PATH + name + ".itb");
    }

    // == Searching for the image which has this ID ==
    ...
    // No image found with the same ID as the one in 'image' parameter
    std::cerr << "[ERROR]: Image '" << name << "' not found" << std::endl;
    return -1;
}
